...
class GithubRepo: CustomStringConvertible {
    ...
    class func fetchRepos(_ settings: GithubRepoSearchSettings, successCallback: @escaping ([GithubRepo], [String]) -> (), error: ((Error?) -> ())?) {
        ...
        manager.get(reposUrl, parameters: params, success: { (operation: AFHTTPRequestOperation, responseObject: Any) in
            if let response = responseObject as? NSDictionary, let results = response["items"] as? NSArray {
                ...
                for result in results as! [NSDictionary] {
                    ...
                    var language = GithubRepo(jsonResult: result).language
                    if language != "" && !languageList.contains(language) {
                        languageList.append(language)
                    }
                }
                successCallback(repos, languageList)
            }
